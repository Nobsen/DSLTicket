...
/**
 * Abstract superclass for all Hibernate-classes in this package. Implementation uses the plain Hibernate 3 API. Using this API
 ...
 * translation.
 * 
 * Note that the {@link Repository} annotation does not only work for Hibernate but also for other data access technologies such
 * as JPA.
 * 
...
public abstract class AbstractDao<T extends DomainObject, ID extends Serializable> implements GenericDao<T, ID> {
    ...
    public T makePersistent(T entity) {
        // Note: Hibernate saveOrUpdate works only with Hibernate CascadeType.SAVE-UPDATE or
        // JPA CascadeType.ALL, not with JPA {CascadeType.PERSIST, CascadeType.MERGE}.
        // save immediately returns an object id, persist does not.
    ...
    public long currentDbTimeInMillis() {
        // Haessliches 'select'-Statement. Aber eine bessere Moeglichkeit, die ein unique Ergebnis, sowohl fuer
        // Tabellen mit 0, 1 und n Eintraegen liefert habe ich nicht gefunden.
        Query query = getSession().createQuery("select coalesce(max(current_timestamp()), current_timestamp()) from " +  getPersistentClass().getName());
        Timestamp result = (Timestamp) query.uniqueResult();
